...
export default {
        ...
        return (
          <div class="tree-item">
            { data.type === 'folder' ? <icon type="ios-folder" color="#2d8cf0" style="margin-right: 10px;" /> : ''}
            {
              isRenaming
                ? <span>
                  <i-input class="tree-rename-input" value={data.title} on-input={this.handleInput} />
  ...
  methods: {
    ...
    handleInput (value) {
      this.currentRenamingIdContent = value
    },
    updateList (list, id) {
      let i = -1
      let len = list.length
      while (++i < len) {
        ...
        if (folderItem.id === id) {
          folderItem.name = this.currentRenamingIdContent
